/**
 * Android emulator driver implementing DeviceDriver interface.
 * Manages full lifecycle: create AVD, boot headless, health check, shutdown, cleanup.
 * Zombie-aware: detects uninterruptible qemu processes and routes around them.
 */
 ...
 * See .planning/phases/33-android-grpc/33-RESEARCH.md §Emulator spawn.
 * Throws on band exhaustion so callers can fall back to scrcpy-only (Wave 4+).
 */
...
export class AndroidEmulatorDriver implements DeviceDriver {
  /** Tracks PID per AVD name. Phase 33: `grpcPort` records the Android EmulatorController port. */
  ...
  constructor(config: AndroidDeviceConfig, logger: pino.Logger) {
    this.config = config;
    this.logger = logger.child({ component: 'android-emulator' });
  }
  ...
  async boot(emulatorId: string, options?: BootOptions): Promise<BootResult> {
    ...
    // Phase 33: allocate distinct gRPC port for the Android EmulatorController.
    // Failure propagates — Wave 4's TS adapter is responsible for scrcpy fallback.
  ...
  async shutdown(emulatorId: string): Promise<void> {
    ...
    if (stat && isZombieStat(stat)) {
      ...
      this.processes.delete(emulatorId);
      // Do NOT throw — let caller proceed with fresh allocation
      return;
    }
  ...
  async isHealthy(emulatorId: string, port?: number | null): Promise<boolean> {
    const info = this.processes.get(emulatorId);
    // Resolution order: tracked process port → explicit port arg → assume
    // emulatorId is the ADB serial (physical devices, which we never boot).
    const serial = info
      ? adbSerial(info.port)
      : (port != null ? adbSerial(port) : emulatorId);
